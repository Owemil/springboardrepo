...
const { authUser, requireLogin, requireAdmin, ensureUserandAdmin } = require('../middleware/auth');
const userPatchSchema = require("../schema/userPatchSchema.json")
...
router.get('/', authUser, requireLogin, async function (req, res, next) {
  try {
    ...
    // BUG #7
    const betterResp = users.map(u => ({
      username: u.username,
      firstName: u.first_name,
      lastName: u.last_name
    }))

    return res.json({ users: betterResp });
  } catch (err) {
    return next(err);
  }
}); // end
 ...
 *
 */
// BUG #2:requireAdmin/ if statement should become seperate middelware
router.patch('/:username', authUser, requireLogin, ensureUserandAdmin, async function (
  req,
  res,
  next
) {
