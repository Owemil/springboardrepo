...
def add_song():
    ...
    if form.validate_on_submit():
        ...
        new_song = Song(title=title,author=author)
...
def add_song_to_playlist(playlist_id):
    ...
    res = (db.session.query(Song.id,Song.title).filter(Song.id.notin_(curr_on_playlist)).all())
    form.song.choices = [(row.id, row.title) for row in res]
